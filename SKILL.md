---
name: review-skill
description: Use when asked to review, audit, or check an Angular feature module/branch in the DemarcoDS / Bancodoc.Interface project — both (A) for design-system consistency in the code (toasts incl. "salvo" form vs "inserido/editado" inline, modals incl. danger removal variant and dialog sizing, button/action naming, form validation & duplicate-name messages, tables incl. first-column-only bold and non-clickable rows, breadcrumbs & app layout, list-screen and form layout, clickable-count→modal links, tooltip-vs-alert), and (B) verifying the change in the running app (stale bundles, change-detection zones, mock-API signals). Reports what is out of standard with the correct pattern and the fix. Triggers on "está no padrão?", "fora do padrão", "padroniza", "revisa o módulo", "review em funcionamento/componentes".
---
...
- **Tamanho do modal de CONTEÚDO (form/upload/detalhe): ⚠️ EM ABERTO — não reporte como desvio até definir.** Escala do DS: `sm` 506 · `md` 714 · `lg` 826 · `xl` 994 · `2xl` 1200 (`libs/ui/src/lib/components/dialog/dialog.component.ts`). Há um conflito real entre duas fontes:
  - **O código (jul/2026):** 32 modais de conteúdo em **`xl`**, incluindo praticamente todo o `servicos/documentos` — o módulo de referência canônica desta skill. Inclui forms de coluna única (`tipo-documento-form-dialog`, `categoria-motivo-form-dialog`), ou seja, `xl` não está reservado a conteúdo largo. Contra 17 em `lg`.
  - **O Figma:** frame de modal de conteúdo em **824 × 580 Hug** ≈ **`lg`** (826).
  Enquanto isso não for decidido com o design, **não migre modais nem aponte `xl` como erro** — o peso do código está em `xl`. O que É seguro apontar: modal cujo `size` claramente não cabe o conteúdo (campo único em `2xl`), e o caso abaixo do `host`.
- **Ao REMOVER conteúdo de um modal** (ex.: tirar um bloco lateral), reavalie o `size` — sobra largura vazia.
